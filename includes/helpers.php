...
// Application base URL helper (works under subdirectory deployments)
// Returns the base path of the application (e.g., '' or '/wavinyacup') by
// removing known role segments from the current script directory.
function app_base_url() {
    // Start from the directory of the current script path
    $base = rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? ''), '/');

    // Remove a known top-level role segment if present to get the app root
    // Example: '/wavinyacup/viewer' -> '/wavinyacup'
    $base = preg_replace('#/(admin|coach|captain|player|viewer|sub_county|ward|auth)$#', '', $base);

    // Normalize root to empty string instead of '/'
    if ($base === '/') {
        $base = '';
    }
...
function has_permission($permission) {
    ...
    $permissions_map = [
        ...
        'viewer' => ['view_dashboard', 'view_all_teams', 'view_all_players', 'view_all_coaches', 'export_team_pdf'],
        'player' => []
    ];
 ...
 * This function should be placed in a helpers file or here.
 * @param int $ward_id
 * @return string
 */
...
function redirect_to_dashboard($role) {
    // Get the base URL for the application (handles subdirectory deployments)
