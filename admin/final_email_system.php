<?php
// final_email_system.php - Alternative email system with multiple providers
ini_set('display_errors', 1);
ini_set('error_reporting', E_ALL);

if (!isset($_GET['access']) || $_GET['access'] !== 'final2025') {
    die('Access denied - use ?access=final2025');
}
...
// Alternative email function using PHP's built-in mail()
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_test_email') {
        try {
            $test_subject = "Wavinyacup Test Email - " . date('H:i:s');
            $test_body = "
            <html>
            <body style='font-family: Arial, sans-serif;'>
                <h2>Test Email from Wavinyacup System</h2>
                <p>This is a test email sent at: " . date('Y-m-d H:i:s') . "</p>
                <p>If you receive this, the email system is working!</p>
            </body>
            </html>";
            
            if (send_simple_email('justusmunyoki@gmail.com', $test_subject, $test_body)) {
                $message = "✅ Test email sent successfully using server mail!";
            } else {
                $message = "❌ Test email failed - server mail() function not working";
            }
        } catch (Exception $e) {
    ...
    if ($action === 'send_bulk_credentials') {
        try {
            ...
            foreach ($all_coaches as $coach) {
                ...
                // Send email using server mail
                if (send_simple_email($coach['email'], $email_subject, $email_body)) {
                    $sent_count++;
                } else {
                    $failed_count++;
                }
...
<head>
    ...
    <title>Final Email System</title>
    <style>
...
<body>
    <div class="container">
        <h1>🚀 Final Email System</h1>
        
        <div class="info-box">
            <h4>📧 Email Method: Server Mail (Alternative to Gmail SMTP)</h4>
            <p>This system uses your server's built-in mail function instead of Gmail SMTP to avoid authentication issues.</p>
        </div>
        
        <?php if ($message): ?>
            <div class="message <?= str_contains($message, 'error') || str_contains($message, 'failed') || str_contains($message, '❌') ? 'error' : '' ?>">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">Server Mail</div>
            </div>
        </div>
